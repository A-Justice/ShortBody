...
namespace ShortBody.UViewModel
{
    public class UserPageViewModel : DashBoardViewModel, IDataErrorInfo
    {
        ...
        private void delete_E(object obj)
        {
            try
            {
                if(allUsersSelectedId == 1)
                {
                    MessageBox.Show("Cannot delete default user", "Invalid Operation", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                if (allUsersSelectedId == null || allUsersSelectedId < 0)
                {
        ...
        public async void SearchUsers()
        {
            ...
            await Task.Run(() =>
            {
                if (SearchBy == "FirstName")
                    searchedUsers = new ObservableCollection<User>(searchedUsers.Where(i => i.FirstName.ToLower() == SearchString.ToLower()));
                else if (SearchBy == "LastName")
                    searchedUsers = new ObservableCollection<User>(searchedUsers.Where(i => i.LastName.ToLower() == SearchString.ToLower()));
            });
